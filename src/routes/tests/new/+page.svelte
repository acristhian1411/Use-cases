<script>
  ...
  /** @type {{ data: { modules: Array<{ id: number, name: string }>, selectedModuleId: number | null } }} */
  let { data } = $props();

  let steps = $state([
    { stepNumber: 1, action: "", expected: "", type: "normal" },
  ]);
  let actors = $state([{ actorName: "" }]);
  let selectedModuleId = $state(
    data.selectedModuleId ||
      (data.modules.length > 0 ? data.modules[0].id : ""),
  );
...
<div class="max-w-4xl mx-auto space-y-6 pb-20">
  <div class="flex items-center gap-4">
    <a
      href={selectedModuleId ? `/modules/${selectedModuleId}` : "/modules"}
      class="p-2 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg transition-colors"
    >
      <ArrowLeft size={20} />
    </a>
    <h1 class="text-2xl font-bold text-white">Create New Test Case</h1>
  </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="excepcion">Exception</option>
                  </select>
                </div>
              </div>
              <input
