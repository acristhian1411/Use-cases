    ...
    >
      ...
      <div class="space-y-2">
        ...
        <select
          id="status"
          name="status"
          value={data.testCase.status || "untested"}
          class="w-full px-4 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all"
        >
      ...
      {#if data.testCase.status === "failed"}
        <div
          class="mt-4 p-4 bg-red-500/5 border border-red-500/20 rounded-xl flex items-center justify-between"
        >
          <span class="text-sm text-red-400"
            >This test case execution failed. Would you like to log a bug?</span
          >
          <a
            href={`/bugs/new?testCaseId=${data.testCase.id}&moduleId=${selectedModuleId}`}
